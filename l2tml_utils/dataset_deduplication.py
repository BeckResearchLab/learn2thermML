...
def _find_cluster_extremes_shared(cluster, jaccard_threshold, k, num_perm):
    ...
    for element1 in cluster:
        code1 = compute_min_hash(_shared_dataset[element1["base_index"]]["protein_seq"], k=k, num_perm=num_perm)
        for element2 in extremes:
            code2 = compute_min_hash(_shared_dataset[element2["base_index"]]["protein_seq"], k=k, num_perm=num_perm)
...
def find_extremes(cluster_list, dataset, jaccard_threshold, k, num_perm):
